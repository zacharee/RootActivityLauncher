...
import tk.zwander.rootactivitylauncher.util.manifestMinAspectRatioCompat
import tk.zwander.rootactivitylauncher.util.map
import tk.zwander.rootactivitylauncher.util.persistableModeToString
import tk.zwander.rootactivitylauncher.util.rMaxAspectRatio
...
private fun processServiceInfo(info: ServiceInfo): List<CharSequence> {
...
private fun processActivityInfo(info: ActivityInfo): List<CharSequence> {
    val sWriter = StringWriter()
    val pWriter = PrintWriter(sWriter)
    val printer = PrintWriterPrinter(pWriter)
...
private fun processPackageInfo(info: PackageInfo): List<CharSequence> {
    val sWriter = StringWriter()
    val pWriter = PrintWriter(sWriter)
    val printer = PrintWriterPrinter(pWriter)
...
private fun formatDump(dump: List<CharSequence>, firstPass: Boolean = true): List<AnnotatedString> {
...
private suspend fun applyQuery(
